...
public class Assembler {
    ...
    private int parseNumber(String input) {
        ...
        if (lowerInput.startsWith("0x")) {
            return Integer.parseInt(lowerInput.substring(2), 16);
        }
        try {
            return Integer.parseInt(input);
        } catch (NumberFormatException e) {
            throw new IllegalArgumentException("Invalid number format: '" + input + "'. Supported formats: decimal (e.g. 10) or hexadecimal (e.g. 0xA).");
        }
    }
     ...
     */
    private Operand getValue(String input) {
        if (input.startsWith("[")) { // Memory address
            ...
            if (upperContent.startsWith("SP")) {
                ...
                if (remaining.startsWith("+") || remaining.startsWith("-")) {
                    ...
                    try {
                        int offset = parseNumber(offsetStr);
                        if (operator == '-') {
                            offset = -offset;
                        }
                        return new Operand(Operand.Type.STACK_OFFSET, offset);
                    } catch (Exception e) {
                        throw new IllegalArgumentException("Invalid offset in [SP +/- x] expression: " + content);
                    }
                }
            }
            ...
            if (character.length() > 1) {
                throw new IllegalArgumentException("Only one character is allowed. Use String instead");
            }
        ...
        for (int i = 0; i < lines.length; i++) {
            ...
            if (labelEnd != -1) {
                ...
                if (labels.containsKey(label.toUpperCase())) {
                    throw new IllegalArgumentException("Duplicate label: " + label + " on line " + (i + 1));
                }
            ...
            switch (instruction.toUpperCase()) {
                case "ORG":
                    Operand orgOp = getValue(operandsString);
                    if (orgOp.type != Operand.Type.NUMBER) {
                        throw new IllegalArgumentException("ORG directive requires a numeric address.");
                    }
                    addressCounter = (Integer) orgOp.value;
                    if (addressCounter < 0 || addressCounter >= code.length) {
                        throw new IllegalArgumentException("ORG address out of memory bounds (0-255).");
                    }
                    break;
                case "DB":
                    Operand p1 = getValue(operandsString);
                    if (p1.type == Operand.Type.NUMBER) {
                        ...
                        throw new IllegalArgumentException("DB does not support this operand");
                    }
                    break;
                case "HLT":
                ...
                case "DIV":
                    ...
                    if (parts.length != 2) {
                        throw new IllegalArgumentException(instruction.toUpperCase() + " requires two operands separated by a comma.");
                    }
                    Operand op1 = getValue(parts[0].trim());
                    Operand op2 = getValue(parts[1].trim());
                    int opCode;
                    ...
                    switch (instruction.toUpperCase()) {
                        case "MOV":
                            if (op1.type == Operand.Type.REGISTER && op2.type == Operand.Type.REGISTER) {
                                ...
                                throw new IllegalArgumentException("MOV does not support this operands");
                            }
                            break;
                        case "ADD":
                            if (op1.type == Operand.Type.REGISTER && op2.type == Operand.Type.REGISTER) {
                                ...
                                throw new IllegalArgumentException("ADD does not support this operands");
                            }
                            break;
                        case "SUB":
                            if (op1.type == Operand.Type.REGISTER && op2.type == Operand.Type.REGISTER) {
                                ...
                                throw new IllegalArgumentException("SUB does not support this operands");
                            }
                            break;
                        case "CMP":
                            if (op1.type == Operand.Type.REGISTER && op2.type == Operand.Type.REGISTER) {
                                ...
                                throw new IllegalArgumentException("CMP does not support this operands");
                            }
                            break;
                        case "AND":
                            if (op1.type == Operand.Type.REGISTER && op2.type == Operand.Type.REGISTER) {
                                ...
                                throw new IllegalArgumentException("AND does not support this operands");
                            }
                            break;
                        case "OR":
                            if (op1.type == Operand.Type.REGISTER && op2.type == Operand.Type.REGISTER) {
                                ...
                                throw new IllegalArgumentException("OR does not support this operands");
                            }
                            break;
                        case "XOR":
                            if (op1.type == Operand.Type.REGISTER && op2.type == Operand.Type.REGISTER) {
                                ...
                                throw new IllegalArgumentException("XOR does not support this operands");
                            }
                            break;
                        case "SHL":
                            if (op1.type == Operand.Type.REGISTER && op2.type == Operand.Type.REGISTER) {
                                ...
                                throw new IllegalArgumentException("SHL does not support this operands");
                            }
                            break;
                        case "SHR":
                            if (op1.type == Operand.Type.REGISTER && op2.type == Operand.Type.REGISTER) {
                                ...
                                throw new IllegalArgumentException("SHR does not support this operands");
                            }
                            break;
                        case "MUL":
                            if (op1.type == Operand.Type.REGISTER && op2.type == Operand.Type.REGISTER) {
                                ...
                                throw new IllegalArgumentException("MUL does not support this operands");
                            }
                            break;
                        case "DIV":
                            if (op1.type == Operand.Type.REGISTER && op2.type == Operand.Type.REGISTER) {
                                ...
                                throw new IllegalArgumentException("DIV does not support this operands");
                            }
                            break;
                        default:
                            throw new IllegalStateException("Unexpected two-operand instruction: " + instruction);
                    }
                    code[addressCounter++] = opCode;
                    addressCounter = addOperand(code, labelReferences, op1, addressCounter);
                    addressCounter = addOperand(code, labelReferences, op2, addressCounter);
                    break;
                case "INC":
                case "DEC":
                case "NEG":
                case "NOT":
                case "POP":
                    op1 = getValue(operandsString);
                    if (op1.type != Operand.Type.REGISTER) {
                        throw new IllegalArgumentException(instruction.toUpperCase() + " does not support this operand");
                    }
                    switch (instruction.toUpperCase()) {
                ...
                case "PUSH":
                    ...
                    if (op1.type == Operand.Type.REGISTER) {
                        ...
                        throw new IllegalArgumentException("PUSH does not support this operand");
                    }
                    code[addressCounter++] = opCode;
                    addressCounter = addOperand(code, labelReferences, op1, addressCounter);
                    break;
                ...
                case "POPF":
                    ...
                    break;
                case "IN":
                    op1 = getValue(operandsString.trim());
                    if (op1.type == Operand.Type.NUMBER) {
                        opCode = Opcodes.IN_IMM8;
                        code[addressCounter++] = opCode;
                        addressCounter = addOperand(code, labelReferences, op1, addressCounter);
                    } else {
                        throw new IllegalArgumentException("IN only supports `IN imm8` format.");
                    }
                    break;
                case "OUT":
                    op1 = getValue(operandsString.trim());
                    if (op1.type == Operand.Type.NUMBER) {
                        ...
                        throw new IllegalArgumentException("OUT only supports `OUT imm8` format.");
                    }
                    break;
                default:
     ...
     */
    private int addOperand(int[] code, Map<Integer, String> labelReferences, Operand operand, int addressCounter) {
        ...
        if (operand.value instanceof String) {
            ...
            throw new IllegalArgumentException("Invalid operand value: " + operand.value);
        }
